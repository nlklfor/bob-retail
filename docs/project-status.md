...
- **Not yet confirmed applied**: `20260825120000_add_product_sku.sql`, `20260826090000_auto_generate_product_sku.sql`, `20260827100000_create_contact_messages.sql`. Until these run, the SKU column/auto-generation and the contact form's real save-to-DB won't work (the pages themselves don't crash — they degrade gracefully, e.g. the contact form shows a real "спробуйте пізніше" error rather than a stack trace).

## What's next / currently blocked

- **Hosting decided, not yet deployed (2026-09-03)**: self-hosted via Docker + Caddy on a Ukrainian VPS (**HostPro VPS-3** recommended — real pricing compared against MiroHost, see `docs/deployment.md`) — client priority is Ukrainian hosting/billing over deploy convenience, so this is deliberately not on Vercel. Full runbook in `docs/deployment.md`; the `Dockerfile`/`docker-compose.yml`/`Caddyfile` at the repo root were built and verified end-to-end locally (real build, real run, real HTTPS via Caddy, real Supabase data) before being documented as working.
